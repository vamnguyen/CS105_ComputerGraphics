...
function create_pyramid(typeMaterial = "Solid") {
  // Create a custom pyramid geometry
  const geometry = new THREE.BufferGeometry();

  // Define pyramid vertices (square base and apex)
  ...
  const pyramid = get_material(geometry, typeMaterial);
  pyramid.position.y = 3;

  initUserData(pyramid, "Pyramid", typeMaterial);

  return pyramid;
}

function create_prism(typeMaterial = "Solid") {
  // Create a triangular prism
  const height = 4;
  const radius = 2;
  const geometry = new THREE.CylinderGeometry(radius, radius, height, 3, 1);

  const prism = get_material(geometry, typeMaterial);
  prism.position.y = 3;

  initUserData(prism, "Prism", typeMaterial);
...
function create_geometry(typeMesh, typeMaterial) {
  switch (typeMesh) {
    case "Cube":
      return create_cube(typeMaterial);
    case "Sphere":
      return create_sphere(typeMaterial);
    case "Cone":
      return create_cone(typeMaterial);
    case "Cylinder":
      return create_cylinder(typeMaterial);
    case "Torus":
      return create_torus(typeMaterial);
    case "Torus Knot":
      return create_torus_knot(typeMaterial);
    case "Teapot":
      return create_teapot(typeMaterial);
    case "Tube":
      return create_tube(typeMaterial);
    case "Tetrahedron":
      return create_tetrahedron(typeMaterial);
    case "Octahedron":
      return create_octahedron(typeMaterial);
    case "Dodecahedron":
      return create_dodecahedron(typeMaterial);
    case "Icosahedron":
      return create_icosahedron(typeMaterial);
    case "Heart":
      return create_heart(typeMaterial);
    case "Pyramid":
      return create_pyramid(typeMaterial);
    case "Prism":
      return create_prism(typeMaterial);
    default:
      return create_cube(typeMaterial);
  }
}
